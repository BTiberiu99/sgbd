package translate

var (
	message = map[string]string{
		"succes_connection":        "Conexiunea a fost createa cu succes",
		"fail_connection":          "Conexiunea a esuat cu eroarea :\n%s",
		"succes_remove_connection": "Conexiunea %s a fost stearsa cu succes",
		"no_connection":            "Trebuie sa stabilit intai o conexiune!",
		"exist_connection":         "Conexiunea %s exista deja !",
		"not_exist_connection":     "Conexiunea %s nu exista! Trebuie sa o creati intai!",
	}
)
